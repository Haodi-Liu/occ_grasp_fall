...
```
occ_grasp_models/eval_ppi.py:main()
  ├─> eval_seed()
...
## 1. occ_grasp_models/eval_ppi.py:main() 详解

**文件**: `occ_grasp_models/eval_ppi.py`

```python
@hydra.main(version_base="1.1", config_name="eval_ppi", config_path="conf")
...
def eval_seed(eval_cfg, weightsdir, logdir, env_device, multi_task, seed, env_config):
    ...
    # 4. 启动评估进程
    aux_eval_cfg = OmegaConf.select(eval_cfg, "aux_eval", default=None)
...
**文件**: `occ_grasp_models/agents/ppi/launch_utils.py`
...
**文件**: `occ_grasp_models/helpers/preprocess_agent.py`
...
**文件**: `occ_grasp_models/helpers/custom_rlbench_env.py`
...
**文件**: `occ_grasp_models/agents/ppi/ppi_agent.py`
...
**文件**: `occ_grasp_models/ppi/policy/ppi.py`
...
```python
# 数据加载 (来自 dataset)
batch['obs']['point_cloud']  # (B, 1, fps_num, Cpc)  # Cpc=3或6
batch['obs']['dino_feature'] # (B, 1, fps_num, 384)
...
> 代码核验注记：训练采样器默认只加载首帧视觉观测（`point_cloud` / `dino_feature` 的 `T_obs=1`），动作序列 horizon 仍可大于 1。
...
**当前常见设置**: 评估脚本通常将 `num_inference_steps` 设为 `1000`（可按速度需求下调）。
