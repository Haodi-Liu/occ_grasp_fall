...
本节对 `/home/hdliu/claude_repo/PPI/ppi` 目录下的组织结构进行系统介绍，帮助快速理解各模块职责并区分命名相似的脚本。
...
**代码路径**: `/home/hdliu/claude_repo/PPI`
...
| 文件路径 | 功能描述 |
|---------|---------|
| `ppi/policy/ppi.py` | PPI策略主类，封装训练和推理逻辑 |
| `ppi/model/diffusion/diffuser_actor_ppi.py` | 扩散头核心实现，关键帧+连续+点流预测 |
| `ppi/model/vision/observation_encoder.py` | 观测编码器，处理点云和语义特征 |
| `ppi/model/vision/semantic_feature_extractor.py` | DINOv2特征提取和3D投影 |
| `ppi/model/vision/pointnet2.py` | PointNet++点云编码器 |
| `ppi/dataset/rlbench2_dataset.py` | RLBench数据集加载 |
| `ppi/common/get_data_keyframe_continuous.py` | 关键帧发现和数据处理 |
| `ddp_train.py` | 分布式训练脚本 |
| `inference-for-rlbench2/agents/ppi/ppi_agent.py` | RLBench评估Agent封装 |
...
**文件**: `ppi/model/vision/semantic_feature_extractor.py`
...
**文件**: `ppi/model/vision/semantic_feature_extractor.py` (行166-192)
...
**文件**: `ppi/model/vision/pointnet2.py` (行253-273)
...
**文件**: `ppi/model/vision/observation_encoder.py` (行51-124)
...
**文件**: `ppi/common/get_data_keyframe_continuous.py` (行117-159)
...
**文件**: `ppi/common/sampler_keyframe_continuous.py` (行9-83)
...
**文件**: `ppi/model/diffusion/diffuser_actor_ppi.py` (行285-436)
...
初始状态:                    目标状态:
   ...
   ●  ●  ●                     ●  ●  ●
   ↓  ↓  ↓
Point Flow = [Δx, Δy, Δz] for each keypoint
```

### 4.2 点流数据准备

**文件**: `ppi/common/get_data_keyframe_continuous.py` (行69-91)
...
**文件**: `ppi/common/sampler_keyframe_continuous.py` (行195-212)
...
**文件**: `ppi/model/diffusion/diffuser_actor_ppi.py` (行459-480)
...
**文件**: `ppi/model/diffusion/diffuser_actor_ppi.py` (行97-134)
...
**文件**: `inference-for-rlbench2/agents/ppi/ppi_agent.py` (行461-557)
...
class PPIAgent:
    def __init__(self, ...):
        ...
        # 初始化3D特征提取器
        self.feature_fusion = Fusion(num_cam=2, feat_backbone='dinov2')

    def act(self, obs):
        # 1. 提取3D语义特征
        dino_feat = self.feature_fusion.extract_semantic_feature_from_ptc(pcd, cam_obs)

        # 2. 构建观测
        obs_dict = {
            'agent_pos': gripper_state,
            'point_cloud': pcd_with_dino,
            'lang': lang_embedding,
            'initial_point_flow': initial_flow_points
        }

        # 3. 模型推理
        result = self.policy.predict_action(obs_dict)
        action = result['action']  # (B, T, 16)

        # 4. 转换为末端执行器位姿
        # 格式: [left_pos(3), left_quat(4), left_gripper(1),
        #        right_pos(3), right_quat(4), right_gripper(1)]
        raw_action = action[0, 0]  # 取第一步动作
```
...
# 转换为RLBench BimanualEndEffectorPoseViaPlanning格式 (16D):
...
**文件**: `inference-for-rlbench2/conf/eval_ppi.yaml`
