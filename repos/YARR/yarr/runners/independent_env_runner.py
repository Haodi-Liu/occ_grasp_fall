    ...
    def start(self, weight,
        ...
        if multi_task:
            eval_env = CustomMultiTaskRLBenchEnv(
                ...
                aux_eval_cfg=aux_eval_cfg,
                dagger_collect_cfg=dagger_collect_cfg)
        else:
        ...
        if isinstance(self._env_device, torch.device):
            ...
            elif self._env_device.type == "cuda":
                if torch.cuda.device_count() > 1:
                    env_device = torch.device(f"cuda:{device_idx}")
                elif self._env_device.index is not None:
                    env_device = torch.device(f"cuda:{self._env_device.index}")
                else:
                    env_device = torch.device("cuda:0")
            else:
