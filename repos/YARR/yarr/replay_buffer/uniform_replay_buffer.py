...
class UniformReplayBuffer(ReplayBuffer):
    ...
    def _add(self, kwargs: dict):
        ...
        with self._lock:
            cursor = self.cursor()
            self._maybe_warn_replay_overflow(cursor)
    ...
    def _maybe_warn_replay_overflow(self, cursor: int):
        """Warn once when buffer starts overwriting old transitions."""
        if self._add_count.value < self._replay_capacity:
            return
        if self._overflow_warning_emitted:
            return
        logging.warning("!" * 88)
        logging.warning(
            "REPLAY BUFFER CAPACITY REACHED: oldest transitions will be overwritten."
        )
        logging.warning(
            "replay_capacity=%d, add_count=%d, next_overwrite_index=%d",
            self._replay_capacity,
            self._add_count.value,
            cursor,
        )
        logging.warning("This warning is emitted once per replay buffer instance.")
        logging.warning("!" * 88)
