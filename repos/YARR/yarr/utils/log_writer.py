...
class LogWriter(object):
    ...
    def __init__(self,
                 ...
                 aux_eval_csv: str = 'aux_eval_data.csv'):
        ...
        if csv_logging:
            ...
            if aux_eval_csv:
                self._aux_prev_row_data = self._aux_row_data = OrderedDict()
                self._aux_csv_file = os.path.join(logdir, aux_eval_csv)
                self._aux_field_names = None
            else:
                self._aux_prev_row_data = None
                self._aux_row_data = None
                self._aux_csv_file = None
                self._aux_field_names = None
    ...
    def add_scalar(self, i, name, value):
        ...
        if self._csv_logging:
            if (self._aux_csv_file is not None and
                    (name.startswith('aux_eval/') or name.startswith('aux_eval_'))):
                if len(self._aux_row_data) == 0:
                    self._aux_row_data['step'] = i
                self._aux_row_data[name] = value.item() if isinstance(
    ...
    def end_iteration(self):
        ...
        # write aux-eval data
        if self._csv_logging and self._aux_csv_file is not None and len(self._aux_row_data) > 0:
            should_write_aux_header = not os.path.exists(self._aux_csv_file)
            with open(self._aux_csv_file, mode='a+') as csv_f:
                names = self._aux_row_data.keys()
                writer = csv.DictWriter(csv_f, fieldnames=names)
                if should_write_aux_header:
                    if self._aux_field_names is None:
                        writer.writeheader()
                    else:
                        if not np.array_equal(self._aux_field_names, self._aux_row_data.keys()):
                            missing_keys = list(set(self._aux_field_names) - set(
                                self._aux_row_data.keys()))
                            for mk in missing_keys:
                                self._aux_row_data[mk] = self._aux_prev_row_data[mk]
                self._aux_field_names = names
                try:
                    writer.writerow(self._aux_row_data)
                except Exception as e:
                    print(e)
            self._aux_prev_row_data = self._aux_row_data
            self._aux_row_data = OrderedDict()
