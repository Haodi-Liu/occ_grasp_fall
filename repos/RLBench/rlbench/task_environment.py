...
class TaskEnvironment(object):
    ...
    def step(self, action, visual_targets=None) -> (Observation, int, bool):
        # returns observation, reward, done, info
        # visual_targets: optional list of visualization spheres (PPI feature)
        #                 if provided, they will be hidden after action execution
        if not self._reset_called:
            raise RuntimeError(
                "Call 'reset' before calling 'step' on a task.")
        self._action_mode.action(self._scene, action)

        # Hide visual targets if provided (PPI visualization feature)
        if visual_targets:
