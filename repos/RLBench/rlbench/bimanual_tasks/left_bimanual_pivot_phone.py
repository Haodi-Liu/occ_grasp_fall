"""
LeftBimanualPivotPhone - 专用于收集 left_grasper 演示的靠墙撬起抓取任务

配合 left_task_design_bimanual.ttt 使用。
基于 BimanualPivotPhone，启用 left_grasper 方案过滤。
...
class LeftBimanualPivotPhone(BimanualTask):
    """
    专用于收集 left_grasper 演示的靠墙撬起抓取任务。

    配合 left_task_design_bimanual.ttt 使用。
    强制使用 left_grasper 方案，过滤掉 right_grasper。

    策略类型: 2 (WallLever)

    路径点方案:
    - right_grasper: 右臂抓取（会被过滤）
      - grasper路径点: waypoint0, 2, 4, 6
      - pusher路径点: waypoint1, 3, 5, 7, 8
    - left_grasper: 左臂抓取（强制使用）
    ...
    def _setup_phased_evaluator(self):
        ...
        # Phase 1: 检查抓取点高度（翘起检测）
        phase1_conditions = [
            GraspPointHeightCondition(
        ...
        logging.info("PhasedSuccessEvaluator initialized successfully for LeftBimanualPivotPhone")
    ...
    def base_rotation_bounds(self):
        ...
        if not hasattr(self, '_base_rotation_offset'):
            self._base_rotation_offset = np.random.choice([0, np.pi], p=[0.3, 0.7])
    ...
    def base_position_bounds(self):
        ...
        return 0.03, 0.03  # x方向±3cm, y方向±3cm
