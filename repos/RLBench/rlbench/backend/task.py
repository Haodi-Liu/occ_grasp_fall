...
class Task(object):
    ...
    def post_placement_setup(self) -> None:
        """Called after the task is randomly placed in the workspace.

        This hook is called AFTER _place_task() but BEFORE validate().
        Use this for any setup that depends on the final scene configuration,
        such as:
        - Selecting arm roles based on waypoint positions
        - Adjusting object positions relative to the randomized scene
        - Any computation that requires knowing the final task pose

        Note: This is NOT called if is_static_workspace() returns True.
        """
        pass
