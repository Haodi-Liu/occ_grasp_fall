...
class Environment(object):
    ...
    def launch(self):
        ...
        # 确定 TTT 文件路径
        if self._ttt_file:
            ttt_path = join(DIR_PATH, self._ttt_file)
        elif self._robot_setup == 'dual_panda':
            ttt_path = join(DIR_PATH, BIMANUAL_TTT_FILE)
        else:
            ttt_path = join(DIR_PATH, TTT_FILE)

        self._pyrep.launch(ttt_path, headless=self._headless)
