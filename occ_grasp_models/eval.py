...
def eval_seed(
    ...
    time_steps = OmegaConf.select(
        train_cfg,
        "method.prev_action_horizon",
        default=OmegaConf.select(train_cfg, "replay.timesteps", default=1),
    )
    ...
    for split in split_n:
        ...
        for e_idx, weight in enumerate(split):
            ...
            dagger_collect_cfg = OmegaConf.select(eval_cfg, "dagger_collect", default=None)
            ...
            # # Clear GPU cache after evaluating each weight
...
@hydra.main(version_base="1.1", config_name="eval", config_path="conf")
