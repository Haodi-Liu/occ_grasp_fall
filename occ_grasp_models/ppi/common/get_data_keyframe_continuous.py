...
class GetDataKeyframeContinuous():
    ...
    def read_pkl(self, file_path):
        with open(file_path, 'rb') as f:
            obj = pickle.load(f)
        task_name = infer_task_name_from_data_path(self.data_path)
    ...
    def process_episodes(self, start, end, skip_ep, kp_num):
        ...
        for episode in range(start, end + 1):
            ...
            if os.path.exists(low_dim_obs_path):
                ...
                stopping_delta = 0.1
                episode_keypoints, openess_keypoints = self.keypoint_discovery_bimanual(low_dim_obs, episode, stopping_delta,kp_num)
                if len(episode_ends) == 0:
                    for keyframe in episode_keypoints:
                        keyframe_indices.append(keyframe)
                    for keyframe in openess_keypoints:
                        openess_indices.append(keyframe)
                    episode_ends.append(len(low_dim_obs))
                else:
                    for keyframe in episode_keypoints:
                        keyframe_indices.append(keyframe + episode_ends[-1])
                    for keyframe in openess_keypoints:
                        openess_indices.append(keyframe + episode_ends[-1])
    ...
    def keypoint_discovery_bimanual(self, low_dim_obs, episode, stopping_delta=0.1,total_kp=10):
        ...
        # print(f"episode_keypoints:{episode_keypoints},episode:{episode}")
        # print(f"Totally {len(episode_keypoints)} keypoints in episode{episode}.")
        # print(f"Totally {len(openess_keypoints)} openess keypoints in episode{episode}.")
