# ACT BC Enc Agent with Keypoint Pose Prediction
#
# Based on ACT_BC_ENC agent with keypoint extensions from KEYPOINT_POSE_INJECTION_PLAN.md
# Key additions:
# 1. Extract keypoint GT from replay_sample in update()
# 2. Pass keypoint_gt and has_affordance to actor
# 3. Extended summaries for keypoint metrics
...
class ActBCEncAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        # forward pass with 2D keypoint GT (Path B)
        ...
        # ====== [Path B] Summaries with 2D keypoint metrics ======
        self._summaries = {
            'loss': loss_dict['total_losses'],
            'l1': loss_dict['l1'],
    ...
    def act(self, step: int, observation: dict,
        ...
        """
        Act with keypoint pose prediction.

        *** End-to-end Inference ***
        Model uses KeypointPosePredictor internally to predict keypoints.
        No external keypoint_gt needed - model is fully autonomous.
        ...
        if self._timestep % query_freq == 0:
            with torch.no_grad():
                ...
                # forward pass - model internally predicts keypoints using KeypointPosePredictor
