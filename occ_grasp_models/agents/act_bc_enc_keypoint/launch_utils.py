...
#
# Modified for ACT_BC_ENC_KEYPOINT: Added keypoint pose data loading
...
from agents.act_bc_enc_keypoint.act_bc_enc_agent import ActBCEncAgent
from helpers.custom_rlbench_env import CustomRLBenchEnv
from helpers.preprocess_agent import PreprocessAgent
from agents.act_bc_enc_keypoint.act_policy import ACTPolicy, CNNMLPPolicy
...
# Keypoint types mapping (for validation and debugging, reference DATA_COLLECTION_GUIDE.md)
KEYPOINT_TYPES = {
...
def create_replay(batch_size: int, timesteps: int,
    ...
    # ====== [Path B] 2D Keypoint Projection Data ======
    # Reference: KEYPOINT_POSE_INJECTION_PLAN.md Section 7
    # Removed 3D pose prediction, only use 2D projection for supervision

    # Affordance existence flag (still needed for visibility loss)
    observation_elements.append(
        ObservationElement('has_affordance', (), np.bool_)
    )

    # 2D projection coordinates for each camera (from scene.py)
    for cname in cameras:
...
def _add_keypoints_to_replay(
    ...
    # ====== [Path B] Read 2D projections from misc ======
    # Reference: KEYPOINT_POSE_INJECTION_PLAN.md Section 7
    # scene.py stores 2D projections as {cam_name}_contact_2d, etc.
