...
class QAttentionPerActBCAgent(Agent):
    def __init__(
        ...
        rotation_resolution: float,
        use_goal_image: bool = True,  # 新增参数
        goal_image_path: str = None,  # 目标图像路径
        lr: float = 0.0001,
        ...
        # 加载目标图像
        if self._use_goal_image and self._goal_image_path:
    ...
    def _load_goal_image(self):
        ...
        if not self._use_goal_image or self._goal_image_path is None:
            print("Goal image loading skipped: use_goal_image=False or goal_image_path=None")
            return False
        
        try:
            if os.path.exists(self._goal_image_path):
                # 定义图像预处理
                ...
                # 加载并预处理图像
                image = Image.open(self._goal_image_path).convert('RGB')
                self._goal_image = transform(image).unsqueeze(0)  # 添加batch维度
                print(f"Successfully loaded goal image from {self._goal_image_path}")
                return True
            else:
                print(f"Goal image not found at {self._goal_image_path}")
                return False
    ...
    def build(self, training: bool, device: torch.device = None):
        ...
        if self._goal_image is not None:
            self._goal_image = self._goal_image.to(device)
        ...
        if self._training:
            ...
            # 只在需要时加载CLIP模型（完全去语言化时可跳过）
            if not getattr(self._perceiver_encoder, 'no_language', False):
                # 加载CLIP模型用于评估时编码语言目标
                ...
                # 创建一个占位符，防止在代码其他部分调用时出错
                self._clip_rn50 = None
                print("CLIP model loading skipped due to no_language=True")
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        bs = pcd[0].shape[0]
        goal_image = None
        if self._use_goal_image and self._goal_image is not None:
            goal_image = self._goal_image.repeat(bs, 1, 1, 1)

        
        # 语言：无语言时使用零占位；否则从回放读取（缺失也回退零）
        if getattr(self._perceiver_encoder, 'no_language', False):
            lang_goal_emb = torch.zeros((bs, 512), device=self._device)
            lang_token_embs = torch.zeros((bs, 77, 512), device=self._device)
        else:
            lang_goal_emb = replay_sample.get("lang_goal_emb", None)
            lang_token_embs = replay_sample.get("lang_token_embs", None)

            def _to_t(x):
                if x is None:
                    return None
                if isinstance(x, torch.Tensor):
                    return x.to(self._device).float()
                return torch.as_tensor(x, device=self._device, dtype=torch.float32)

            lang_goal_emb = _to_t(lang_goal_emb)
            if lang_goal_emb is None:
                lang_goal_emb = torch.zeros((bs, 512), device=self._device)

            lang_token_embs = _to_t(lang_token_embs)
            if lang_token_embs is None:
                lang_token_embs = torch.zeros((bs, 77, 512), device=self._device)
        ...
        q, voxel_grid = self._q(
            ...
            lang_goal_emb,
            lang_token_embs,
            goal_image=goal_image,              # 显式传 goal image
            bounds=bounds,                      # 显式传 bounds
            prev_bounds=prev_layer_bounds,      # 显式传 prev bounds
    ...
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        ...
        # 当使用去语言化模式时，可以选择性跳过CLIP处理
        goal_image = None
        if self._use_goal_image and self._goal_image is not None:
            goal_image = self._goal_image.to(self._device)
