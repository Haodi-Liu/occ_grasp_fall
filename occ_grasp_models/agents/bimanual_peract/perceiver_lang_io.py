...
class PerceiverVoxelLangEncoder(nn.Module):
    def __init__(
        ...
        channel_count = 1  # voxel features
        if self.low_dim_size > 0:
            channel_count += 1  # proprio features are concatenated on the grid
        if self.use_goal_image:
            channel_count += 1  # goal image features are concatenated on the grid
        elif (not self.no_language) and self.lang_fusion_type == "concat":
            channel_count += 1  # concat fusion tiles language onto the grid

        self.input_dim_before_seq = self.im_channels * channel_count
        ...
        if self.use_goal_image:
            self.goal_image_encoder = nn.Sequential(
                nn.Conv2d(3, 32, kernel_size=8, stride=4, padding=2),  # 224->56
                nn.ReLU(inplace=True),
                nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=1),  # 56->28
                nn.ReLU(inplace=True),
                nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),  # 28->28
                nn.ReLU(inplace=True),
                nn.AdaptiveAvgPool2d((1, 1)),  # 全局平均池化
                nn.Flatten(),
                nn.Linear(64, self.im_channels)  # 映射到与其他特征相同的维度
            )

        # learnable positional encoding
        if self.pos_encoding_with_lang:
            pos_encoding_len = spatial_size**3
            if not self.no_language and not self.use_goal_image:
                pos_encoding_len += lang_max_seq_len
            self.pos_encoding = nn.Parameter(
                torch.randn(1, pos_encoding_len, self.input_dim_before_seq)
            )
        else:
    ...
    def forward(
        ...
        # 处理目标图像 - 只在启用且有编码器时处理
        if self.use_goal_image:
            if goal_image is not None and hasattr(self, 'goal_image_encoder'):
                # 使用实际的目标图像编码器
                goal_feat = self.goal_image_encoder(goal_image)
                goal_feat = goal_feat.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1, 1, d, h, w)
                ins = torch.cat([ins, goal_feat], dim=1)
            else:
                # 使用零张量作为占位符（保持维度一致性）
                goal_feat = torch.zeros(b, self.im_channels, d, h, w, device=device, dtype=ins.dtype)
                ins = torch.cat([ins, goal_feat], dim=1)

        # 处理语言 - 只在启用且不使用目标图像时处理
        if self.no_language:
            # 完全去语言化模式 - 将语言特征清零
            if lang_goal_emb is not None:
        ...
        if (self.lang_fusion_type == "concat" and 
            not self.no_language and 
            not self.use_goal_image and
            hasattr(self, 'lang_preprocess')):
            lang_emb = lang_goal_emb.to(dtype=ins.dtype)
        ...
        if (self.lang_fusion_type == "seq" and 
            not self.no_language and 
            not self.use_goal_image and
            hasattr(self, 'lang_preprocess')):
            if not self.no_language:
        ...
        if self.lang_fusion_type == "seq" and not self.no_language:
