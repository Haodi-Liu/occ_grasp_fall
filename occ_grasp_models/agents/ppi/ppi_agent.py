...
class PPIAgent(Agent):
    ...
    def _to_fusion_uint8_numpy(self, value):
        if torch.is_tensor(value):
            value = value.detach().cpu().clamp(0, 255).round().to(torch.uint8).numpy()
        else:
            value = np.clip(np.rint(value), 0, 255).astype(np.uint8, copy=False)
        return np.ascontiguousarray(value)

    def _to_fusion_float32_numpy(self, value):
        if torch.is_tensor(value):
    ...
    def get_dino_feature(self, ptc, all_cam_images, depth_image_lst, cameras_extrinsics, cameras_intrinsics):
        B = ptc.shape[0]
        T = ptc.shape[1]
        dino_feature = torch.zeros((B, T, self.fps_num, 384), device=self._device)
        all_cam_images = all_cam_images.permute(0, 1, 2, 4, 5, 3)
        for b in range(B):
            for t in range(T):
                pointcloud = ptc[b,t]
                obs ={
                    'color': self._to_fusion_uint8_numpy(all_cam_images[b, t]),
                    'depth': self._to_fusion_float32_numpy(depth_image_lst[b, t]),
                    'pose': self._to_fusion_float32_numpy(cameras_extrinsics[b, t]),
                    'K': self._to_fusion_float32_numpy(cameras_intrinsics[b, t])
                }
