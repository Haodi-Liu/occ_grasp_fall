...
def create_replay(cfg, replay_path):
    no_language = getattr(cfg.method, "no_language", False)
...
def create_bimanual_replay(
    ...
    if no_language:
        observation_elements.append(ReplayElement("task", (), str))
    else:
        observation_elements.extend(
            [
...
def _add_keypoints_to_replay(
    ...
    for k, keypoint in enumerate(episode_keypoints):
        ...
        if robot_name == "bimanual":
            ...
            if not no_language:
                final_obs["lang_goal"] = np.array([description], dtype=object)
        else:
            final_obs = {
    ...
    # Remove depth data from obs_dict_tp1 as replay buffer doesn't expect it.
    keys_to_remove = [k for k in obs_dict_tp1.keys() if "_depth" in k]
    for key in keys_to_remove:
        del obs_dict_tp1[key]
...
def fill_replay(
    ...
    if clip_model is None and not getattr(cfg.method, "no_language", False):
