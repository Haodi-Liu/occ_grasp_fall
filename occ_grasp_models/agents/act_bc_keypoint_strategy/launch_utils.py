...
from agents.act_bc_keypoint_strategy.act_bc_keypoint_strategy_agent import ActBCKeypointStrategyAgent
from agents.act_bc_keypoint_strategy.act_policy import ACTPolicy
...
# === Strategy and Phase type mappings (for validation/clamping) ===
STRATEGY_TYPES = {
    0: 'EdgeHang',
    1: 'WallLever',
    2: 'PressTilt'
...
def create_replay(batch_size: int, timesteps: int,
    ...
    # === Strategy/Phase labels ===
    observation_elements.append(ObservationElement('strategy_type', (), np.int32))
    observation_elements.append(ObservationElement('phase_type', (), np.int32))
...
def _add_keyframes_to_replay(keypoint_idx: int,
    ...
    # ===== Strategy/Phase labels (current keyframe) =====
    strategy_type_1based = misc.get('strategy_type', 1)
    strategy_type = np.int32(strategy_type_1based - 1)
    obs_dict['strategy_type'] = strategy_type
    phase_type_1based = misc.get('phase_type', 1)
    phase_type = np.int32(phase_type_1based - 1)
    obs_dict['phase_type'] = phase_type

    if strategy_type < 0 or strategy_type >= len(STRATEGY_TYPES):
        logging.warning(f"Invalid strategy_type {strategy_type} at step {k}, clamping to valid range")
        strategy_type = np.int32(max(0, min(strategy_type, len(STRATEGY_TYPES) - 1)))
        obs_dict['strategy_type'] = strategy_type
    if phase_type < 0 or phase_type >= len(PHASE_TYPES):
        logging.warning(f"Invalid phase_type {phase_type} at step {k}, clamping to valid range")
...
def create_agent(cfg: DictConfig):
    ...
    bc_agent = ActBCKeypointStrategyAgent(
