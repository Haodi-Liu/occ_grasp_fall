...
"""
DETR-VAE model for ACT_BC_KEYPOINT.

This keypoint-only version integrates:
1. KeypointPosePredictor for contact/grasp/affordance 2D localization (Memory injection)
2. Dual-backbone conditioning with optional gripper tokens
3. Conditional encoder with keypoint/gripper tokens (no strategy/phase query injection)
"""
import torch
...
class DETRVAE(nn.Module):
    """DETR-VAE with keypoint/gripper memory injection (no strategy/phase query injection)."""
    ...
    def __init__(self, backbones, transformer, encoder, state_dim,
        ...
        Parameters:
            ...
            transformer: torch module of the transformer architecture (supports memory injection)
        ...
        if backbones is not None:
            ...
            raise ValueError("ACT_BC_KEYPOINT requires visual backbones for action branch.")
        ...
        if condition_encoder and backbones is not None:
            ...
            # === Type embeddings ===
            # With gripper tokens:
            # 0=CLS, 1=qpos, 2=kp_contact, 3=kp_grasp, 4=kp_affordance,
            # 5=gripper_r, 6=gripper_l, 7=image, 8=action
            # Without gripper tokens:
            # 0=CLS, 1=qpos, 2=kp_contact, 3=kp_grasp, 4=kp_affordance, 5=image, 6=action
            if encoder_use_type_embed:
                num_types = 9 if self.use_gripper_token else 7
                self.encoder_type_embed = nn.Embedding(num_types, hidden_dim)

            # Position table for encoder sequence
            max_img_tokens = len(camera_names) * 64 + 100
            # 1(CLS) + 1(qpos) + 3(keypoints) + 2(grippers) + img + action
            extra_gripper = 2 if self.use_gripper_token else 0
            encoder_pos_table_size = 1 + 1 + 3 + extra_gripper + max_img_tokens + num_queries + 50
            ...
            # Input: qpos + img + contact + grasp + affordance (+ gripper_r + gripper_l)
            prior_in_dim = hidden_dim * (7 if self.use_gripper_token else 5)
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None,
        ...
        """
        Forward pass with keypoint/gripper conditioning.
        ...
        if self.condition_encoder and self.backbones is not None:
            ...
            # === Step 2: Keypoint Prediction (visual-only) ===
            # KeypointPosePredictor expects: visual_feat, img_tokens
            ...
            # === Step 3: Build keypoint tokens (matches KEYPOINT's logic) ===
            ...
            # === Step 4: Build gripper tokens (optional) ===
            ...
            if is_training:
                ...
                # Encoder input: [CLS, qpos, kp_contact, kp_grasp, kp_aff, (gripper_r, gripper_l), img, actions]
                encoder_chunks = [
                ...
                if self.use_gripper_token:
                    encoder_chunks.extend([
                        gripper_r_token,  # (1, B, D) - type 5
                        gripper_l_token,  # (1, B, D) - type 6
                    ])
                encoder_chunks.extend([
                    img_embed,          # (N_img, B, D) - type 7/5
                    action_embed        # (N_act, B, D) - type 8/6
                ])
                ...
                if self.encoder_use_type_embed:
                    type_blocks = [
                        ...
                        torch.full((1, bs), 2, dtype=torch.long, device=qpos.device),       # kp_contact
                        torch.full((1, bs), 3, dtype=torch.long, device=qpos.device),       # kp_grasp
                        torch.full((1, bs), 4, dtype=torch.long, device=qpos.device),       # kp_affordance
                    ]
                    if self.use_gripper_token:
                        type_blocks.extend([
                            torch.full((1, bs), 5, dtype=torch.long, device=qpos.device),   # gripper_r
                            torch.full((1, bs), 6, dtype=torch.long, device=qpos.device),   # gripper_l
                        ])
                        img_type_id = 7
                        act_type_id = 8
                    else:
                ...
                # Fixed tokens: CLS+qpos+keypoints(+grippers)+img
                n_fixed = (7 if self.use_gripper_token else 5) + N_img
