# ACT Policy for Keypoint-only Fusion Agent
#
# Components:
...
from agents.act_bc_keypoint.detr.build import build_ACT_model_and_optimizer, build_CNNMLP_model_and_optimizer
...
class ACTPolicy(nn.Module):
    def __init__(self, args):
        ...
        if kp_ckpt:
            ...
            raise RuntimeError("predictor_ckpt_path is deprecated. Provide predictor_keypoint_ckpt_path.")
    ...
    def forward(self, qpos, image, actions=None, is_pad=None,
        ...
        if actions is not None:  # training time
            if self.condition_encoder and not self._predictor_ckpt_loaded:
                raise RuntimeError(
                    "predictor_keypoint_ckpt_path is required for ACT_BC_KEYPOINT training."
                )
            ...
            (a_hat, is_pad_hat, (mu, logvar),
             keypoint_2d_pred, affordance_visible, affordance_visible_logits) = self.model(
            ...
            # Model internally uses predictor path for condition injection
            (a_hat, _, (_, _), _, _, _) = self.model(
    ...
    def predict_with_aux(self, qpos, image,
        ...
        Returns:
            ...
            aux: dict with keypoints and affordance visibility
        """
        env_state = None
        (a_hat, _, (_, _),
         keypoint_2d_pred, affordance_visible, _) = self.model(
            qpos, image, env_state,
            camera_extrinsics=camera_extrinsics,
            camera_intrinsics=camera_intrinsics,
            gripper_pose_world=gripper_pose_world
