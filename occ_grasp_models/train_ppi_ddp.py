...
class TrainPPIWorkspace:
    ...
    def save_checkpoint(self, path=None, tag='latest', use_thread=False):
        ...
        checkpoint_dict = {
            ...
            # Checkpoints are saved at epoch end, before the in-memory counters are
            # incremented for the next epoch. Persist the next training position so
            # resume starts from the correct epoch instead of replaying the last one.
    ...
    def load_checkpoint(self, path=None, tag='latest', **kwargs):
        ...
        resume_state_version = checkpoint_dict.get("resume_state_version", 1)
        if resume_state_version >= RESUME_STATE_VERSION:
            self.global_step = checkpoint_dict['global_step']
            self.epoch = checkpoint_dict['epoch']
        else:
            # Legacy checkpoints stored the just-finished epoch/step. Shift them to
            # the next training position so existing runs can resume safely.
            self.global_step = checkpoint_dict['global_step'] + 1
