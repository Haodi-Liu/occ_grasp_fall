...
torchrun --nnodes 1 --nproc_per_node $ngpus --master_port 10006 train_ppi_ddp.py \
    task='four_tasks' \
    name='train_ppi_ddp' \
    addition_info='20260413_joint4' \
    wandb_name='ppi_four_tasks' \
    logging.mode=online \
    n_obs_steps=1 \
    n_action_steps=54 \
    ...
    task.dataset.stats_filepath='data/training_processed/norm_stats/norm_stats_bimanual_four_tasks_rgb_pcd_rps6144_keyframe_continuous_world_ordered_rps200.pth' \
    horizon_keyframe=4 \
