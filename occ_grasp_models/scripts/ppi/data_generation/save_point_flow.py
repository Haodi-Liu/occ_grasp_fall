...
from ppi.common.object_pose_from_contact import (
    infer_task_name_from_data_path,
    maybe_patch_low_dim_obs,
)


def save_float32_npy(path, array):
...
class GetPointFlow():
    ...
    def read_pkl(self, file_path):
        with open(file_path, 'rb') as f:
            obj = pickle.load(f)
        task_name = self.task or infer_task_name_from_data_path(self.data_path)
        return maybe_patch_low_dim_obs(file_path, obj, task_name)
    ...
    def process_episodes(self, start, end, cameras, pcd_type):
        ...
        for episode in range(start, end + 1):
            ...
            # set_trace()
            thirdview_point_cloud = np.array(thirdview_point_cloud, dtype=np.float32)
            thirdview_point_cloud = torch.tensor(thirdview_point_cloud, dtype=torch.float32)
            
            T, total_points, _ = thirdview_point_cloud.shape
            # set_trace()
            ps_num = int(pcd_type.split('ps')[-1])
            if total_points < ps_num:
                print(f"Warning: episode {episode} has only {total_points} points "
                      f"(need {ps_num}), using replacement sampling.")
            rand_idx = np.random.choice(total_points, int(ps_num), replace=total_points < ps_num)
            ...
            for t in range(T):
                ...
                save_float32_npy(pcd_folder, sampled_pc[t])
                # set_trace()

        return sampled_pc
    
if __name__ == "__main__":
    # Default config aligned with PPI_OCC_GRASP_COMPLETE_GUIDE.md section 6.3.
    ...
    get_point_flow.process_episodes(0, 149, cameras, pcd_type)
